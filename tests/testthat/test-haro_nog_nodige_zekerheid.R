# Waarden overgenomen uit HARo B7.3.4, de tabel waarvan de bovenste regel luidt
# "Benodigde betrouwbaarheid door het uitvoeren van detailcontroles na inschatten
# van RAM (IR en IBR) en CAR".
# De '-' uitkomsten zijn vervangen door de waarden zoals die uit de functie komen.
#
# Merk op dat er positieve waarden uit de functie komen
# voor alle gevallen behalve de situaties waarvoor alledrie de risico's op
# laag staan. Vanaf 0.53 wordt daarvoor een "-" ingevuld in de tabel.
# De opmerking in HARo B7.3.4 onder de tabel dat er voor de waarden met "-"
# niet effectief met een statistische steekproef zekerheid kan worden verkregen
# lijkt me onjuist. Mogelijk wordt die opmerking ingegeven doordat
# de resulterende steekproefgroottes klein zijn, en onterecht de gedachte bestaat
# dat steekproefgroottes kleiner dan bijvoorbeeld 20 steken niet bruikbaar zijn.
#
test_that("HHH == 0.95", {
...
test_that("MLL == - (0.07)", {
  expect_equal(round(haro_nog_nodige_zekerheid(ihr = 'M', ibr = 'L', car = 'L'), digits = 2), 0.07)
})
...
test_that("LML == - (0.04)", {
  expect_equal(round(haro_nog_nodige_zekerheid(ihr = 'L', ibr = 'M', car = 'L'), digits = 2), 0.04)
})
...
test_that("LLL == - (0.00)", {
  expect_equal(round(haro_nog_nodige_zekerheid(ihr = 'L', ibr = 'L', car = 'L'), digits = 2), 0.00)
})
